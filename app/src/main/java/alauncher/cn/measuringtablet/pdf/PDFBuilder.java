...
public class PDFBuilder extends PdfPageEventHelper {
    ...
    public void setFooters(String left, String mid, String right) {
        footerLeft = left;
        footerMid = mid;
        footerRight = right;
    }
    ...
    public void addPage(PdfWriter writer, Document document) {
        ...
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(headerLeft, fontDetail),
                document.left(), document.top() + 20, 0);

        // 写入中间页眉;
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(headerMid, fontDetail),
                (document.rightMargin() + document.right()
                        + document.leftMargin() - document.left() - bf.getWidthPoint(headerMid, presentFontSize)) / 2.0F + 20F, document.top() + 20, 0);

        // 写入右页眉;
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(headerRight, fontDetail),
                document.right(), document.top() + 20, 0);


        // 1.写入左页脚;
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(footerLeft, fontDetail),
                document.left(), document.bottom() - 20, 0);

        // 写入中间页左;
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(footerMid, fontDetail),
                (document.rightMargin() + document.right()
                        + document.leftMargin() - document.left() - bf.getWidthPoint(footerMid, presentFontSize)) / 2.0F + 20F, document.bottom() - 20, 0);

        // 写入右页左;
        ColumnText.showTextAligned(writer.getDirectContent(),
                Element.ALIGN_LEFT, new Phrase(footerRight, fontDetail),
